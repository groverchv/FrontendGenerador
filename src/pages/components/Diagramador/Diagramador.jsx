...
const Diagramador = forwardRef(function Diagramador(
    { projectId, projectName, sock, setLoading, setSaving }, // Aceptar setLoading y setSaving como props
    ref
) {
    // Estado base
    ...
    const {
        ...
        publishSnapshot,
        versionRef,
        setLoading, // Pasar setter de loading
        setSaving, // Pasar setter de saving
    });
        ...
        <div className="w-full h-[calc(100vh-56px)] md:grid md:grid-cols-[1fr_420px] overflow-hidden relative">
            ...
            <PanelLateral
                ...
                onChangeName={(name) =>
                    ...
                    setNodes((ns) =>
                        ns.map((n) =>
                            n.id === selectedId ? { ...n, data: { ...n.data, label: name } } : n
                        )
                    )
                }
                onAddAttr={(attr) =>
                ...
                onRelacionNM={({ aId, bId, nombreIntermedia }) => {
                    ...
                    setNodes((ns) =>
                        ns.concat({
                            ...
                            data: {
                                label:
                                    nombreIntermedia || `${nodeA.data.label}_${nodeB.data.label}`,
                                attrs: [],
                                usage: {
                                    target: {
                                        tl: 0,
